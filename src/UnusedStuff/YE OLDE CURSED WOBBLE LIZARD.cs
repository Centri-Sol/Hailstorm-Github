	...
	internal static void Apply()
	{
        IL.OverseerAbstractAI.HowInterestingIsCreature += il =>
        {
            ...
            && label != null)
            {
                ...
                c.EmitDelegate((AbstractCreature testCrit) => testCrit.creatureTemplate.type == HSEnums.CreatureType.FreezerLizardLizard);
                c.Emit(OpCodes.Brtrue, label);
            }
            else
                Debug.LogError("Couldn't ILHook OverseerAbstractAI.HowInterestingIsCreature!");
        };
        On.LizardLimb.ctor += (orig, self, owner, connectionChunk, num, rad, sfFric, aFric, huntSpeed, quickness, otherLimbInPair) =>
        {
            ...
            if (owner is LizardGraphics l && l.lizard?.Template.type == HSEnums.CreatureType.FreezerLizardLizard)
            {
        ...
        On.LizardVoice.GetMyVoiceTrigger += (orig, self) =>
        {
            ...
            if (self.lizard is Lizard l && l.Template.type == HSEnums.CreatureType.FreezerLizardLizard)
            {
        ...
        {
            if (type == HSEnums.CreatureType.FreezerLizardLizard)
            {
        ...
        On.Lizard.ctor += (orig, self, abstractCreature, world) =>
        {
            ...
            if (self.Template.type == HSEnums.CreatureType.FreezerLizardLizard)
            {
        ...
        {
            orig(self, ow);
            if (self.lizard.Template.type == HSEnums.CreatureType.FreezerLizardLizard)
            {
                Random.State state = Random.state;
