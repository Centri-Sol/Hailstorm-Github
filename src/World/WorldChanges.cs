...
namespace Hailstorm
{
    internal class WorldChanges
    {
        ...
        {
            orig(pearl, eu);
            if (IsIncanStory(pearl.room.game) && pearl.AbstractPearl.dataPearlType == MoreSlugcatsEnums.DataPearlType.RM && (!ModManager.MMF || !MMF.cfgKeyItemTracking.Value) && pearl.room.game.session is StoryGameSession SGS && Dialogue.firstCycleSeeingMusicPearl > 0)
            {
