...
internal class Dialogue
{
    ...
    {
        return (RWG != null && RWG.IsStorySession && RWG.StoryCharacter == HailstormSlugcats.Incandescent);
    }
    ...
    {            
        orig(gConv);
        if (gConv.currentSaveFile == HailstormSlugcats.Incandescent && (gConv.id == MoreSlugcatsEnums.ConversationID.Ghost_MS || gConv.id == Conversation.ID.Ghost_CC || gConv.id == Conversation.ID.Ghost_SI || gConv.id == Conversation.ID.Ghost_LF || gConv.id == Conversation.ID.Ghost_SB))
        {
            RemoveEvents(gConv);
    ...
    {
        orig(moon, oracle);
        if (IsRWGIncan(oracle.room.game))
        {
    ...
    {
        orig(moon, eu);
        if (!IsRWGIncan(moon.oracle.room.game))
        {
            return;
        }
    ...
    {
        if (moon.oracle.room.game.StoryCharacter == HailstormSlugcats.Incandescent)
        {
            incPresent = false;
            if (moon?.oracle?.room?.abstractRoom is not null)
            {
                ...
                {
                    if (absCtr != null && absCtr.realizedCreature is Player plr && CWT.PlayerData.TryGetValue(plr, out HailstormSlugcats player) && player.isIncan)
                    {
                        incPresent = true;
                        break;
                    }
    ...
    public static void NewMoonDialogue(On.SLOracleBehaviorHasMark.MoonConversation.orig_AddEvents orig, SLOracleBehaviorHasMark.MoonConversation mConv)
    {
        ...
        if (mConv.currentSaveFile == HailstormSlugcats.Incandescent)
        {
    ...
    { // Delicious hardcoded timings, oh boy.
        if (dEvent.owner.currentSaveFile == HailstormSlugcats.Incandescent && dEvent.owner is not null)
        {
