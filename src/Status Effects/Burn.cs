...
public class Burn : Debuff
{
    ...
    public override void DebuffVisuals(Creature victim, bool eu)
    {
        ...
        int burnCount = 0;
        if (CWT.AbsCtrData.TryGetValue(victim.abstractCreature, out CWT.AbsCtrInfo aI) && aI.debuffs is not null)
        {
    ...
    public static bool IsCreatureBurning(Creature ctr)
    {
        ...
        if (CWT.AbsCtrData.TryGetValue(ctr.abstractCreature, out CWT.AbsCtrInfo aI) && aI.debuffs is not null && aI.debuffs.Count > 0)
        {
            foreach (Debuff debuff in aI.debuffs)
            {
