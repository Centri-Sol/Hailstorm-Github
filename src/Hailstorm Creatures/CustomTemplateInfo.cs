...
public static class CustomTemplateInfo
{
    ...
    {
        return
            creatureType == HSEnums.CreatureType.IcyBlueLizard ||
    ...
    {
        return
            (creature is Player player && player.IsIncan(out IncanInfo _)) ||
    ...
    public static class DamageResistances
    {
        ...
        {
            // Bugs and plants are generally weak to the HSEnums.DamageTypes.Cold.

            // Base-game creatures
            if (type == CreatureTemplate.Type.PoleMimic)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.5f; // +100% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.75f; // +33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.75f; // +33% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 2 / 3f; // +50% stun
            }
            else if (type == CreatureTemplate.Type.DropBug)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.8f; // +25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.8f; // +25% stun
            }
            else if (type == CreatureTemplate.Type.SmallCentipede)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.6f; // +66% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.5f; // +100% stun
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 4 / 7f; // +75% stun
                                                                 // Stun is further increased by 1 second in Centipede.Violence.
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.8f;  // +25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 8 / 11f; // +37.5% stun
                                                                  // Stun is further increased by 1 second in Centipede.Violence.
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 1 / 3f; // +200% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 1 / 3f; // +200% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.83f; // +20% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.83f; // +20% stun
            }
            else if (type == CreatureTemplate.Type.BrotherLongLegs)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 0.25f; // +300% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.25f; // +300% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 1.5f; // -33% damage
                                                               // Stun is unchanged.
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.BigJelly)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2f; // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 2f; // -50% stun
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.MotherSpider)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 1.25f; // -20% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 1.25f; // -20% stun
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.Yeek)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2f;   // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 4 / 3f; // -25% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 4 / 3f; // -25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 2f;   // -50% stun
            }
            else if (type == HSEnums.CreatureType.PeachSpider)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 1.50f; // -33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 1.25f; // -20% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2f;  // -50% stun
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 5f;  // -80% stun
            }

            /*else if (type == HailstormEnums.BezanBud)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 2f;  // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 10f; // -90% stun
            }*/

            else if (type == HSEnums.CreatureType.Chillipede)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 100f; // -99% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 100f; // -99% stun
            }
            ...
            //____________________
            // Other modded creatures
            else if (type == new CreatureTemplate.Type("InfantAquapede"))
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 4/3f; // -25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 0.8f; // +25% stun
            }
        ...
        public static void AddCreatureHeatResistances(CreatureTemplate temp, CreatureTemplate.Type type)
        {
            if (type == CreatureTemplate.Type.PoleMimic)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.5f; // +100% stun
            }
            else if (type == CreatureTemplate.Type.TentaclePlant)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.75f; // +33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.75f; // +33% stun
            }
            else if (type == CreatureTemplate.Type.BigNeedleWorm) // Adult Noodlefly
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.75f; // +33% damage
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 2 / 3f; // +50% stun
            }
            else if (type == CreatureTemplate.Type.DropBug)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.8f; // +25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.8f; // +25% stun
            }
            else if (type == CreatureTemplate.Type.SmallCentipede)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 4 / 3f; // -33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 4 / 3f; // -33% stun
            }
            else if (type == CreatureTemplate.Type.Centipede)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2f; // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 2f; // -50% stun
            }
            else if (type == CreatureTemplate.Type.Centiwing)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2f; // -50% damage
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 3f; // -66% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 3f; // -66% stun
            }
            else if (type == CreatureTemplate.Type.Snail)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2f;  // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 10f; // -90% stun
            }
            else if (type == CreatureTemplate.Type.Leech) // Normal red leech
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 1 / 3f; // +200% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 1 / 3f; // +200% stun
            }
            else if (type == CreatureTemplate.Type.Spider) // The little baby Coalescipede spiders
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
            ...
            else if (temp.TopAncestor().type == CreatureTemplate.Type.DaddyLongLegs)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.25f; // +300% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.25f; // +300% stun
            }
            //____________________
            // Downpour creatures
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.AquaCenti)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 5f; // -80% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 5f; // -80% stun
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.BigJelly)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2f; // -50% damage
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 10000f; // -99.99% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 3f;     // -66.66% stun
            }
            //____________________
            // Hailstorm creatures
            if (temp.TopAncestor().type == HSEnums.CreatureType.SnowcuttleTemplate)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 1.5f; // -33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.5f; // +100% stun
            }
            else if (type == HSEnums.CreatureType.PeachSpider)
            {
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 4f; // -75% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 4f; // -75% stun
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.5f;  // +100% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.25f; // +300% stun
            }
            else if (type == HSEnums.CreatureType.Luminescipede)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 1.25f; // -20% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 1.25f; // -20% stun
            }

            /*else if (type == HailstormEnums.Strobelegs)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 5/3f; // -40% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 5/3f; // -40% stun
            }*/
            //____________________
            // Other modded creatures
            else if (type == new CreatureTemplate.Type("InfantAquapede"))
        ...
        public static void AddCreatureVenomResistances(CreatureTemplate temp, CreatureTemplate.Type type)
        {
            ...
            if (type == CreatureTemplate.Type.PoleMimic)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 10f; // -90% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 10f; // -90% damage
            }
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 10000f; // -99.99% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 10000f; // -99.99% stun
            }
            else if (temp.TopAncestor().type == CreatureTemplate.Type.DaddyLongLegs)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 1 / 3f; // +200% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 2 / 3f; // +50% stun
            }
            //____________________
            // Downpour creatures
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.MotherSpider)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 4 / 3f; // -25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 4 / 3f; // -25% stun
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.StowawayBug)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 2.0f; // -50% damage
                                                                // Stun is unchanged
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.MirosVulture)
            {
                // Damage is unchanged
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 1.25f; // -20% stun
            }
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.FireBug)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 0.8f; // +25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 0.6f; // +66% stun
            }
            ...
            else if (temp.TopAncestor().type == HSEnums.CreatureType.SnowcuttleTemplate)
            {
                ...
                {
                    temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 4f;   // -75% damage
                    temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 100f; // -99% stun
                }
                else if (type == HSEnums.CreatureType.SnowcuttleMale)
                {
                    temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 4 / 3f; // -25% damage
                    temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 1.5f; // -33% stun
                }
            }
            else if (type == HSEnums.CreatureType.IcyBlueLizard)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 1.25f; // -20% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 1.25f; // -20% stun
            }
            else if (type == HSEnums.CreatureType.FreezerLizard)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 2f; // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 2f; // -50% stun
            }
            else if (type == HSEnums.CreatureType.GorditoGreenieLizard)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 2f;   // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 0.4f; // +150% stun
            }
            else if (type == HSEnums.CreatureType.PeachSpider)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 100f; // -99% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 100f; // -99% stun
            }
            else if (type == HSEnums.CreatureType.Cyanwing)
            {
            ...
            /*else if (type == HailstormEnums.BezanBud)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 2/3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 2/3f; // +50% stun
            }*/
            ...
            {
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 2f;   // -50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 0.5f; // +100% stun
            }
        ...
        public static void AddLizardDamageResistances(ref CreatureTemplate temp, CreatureTemplate.Type type)
        {
            ...
            else if (type == CreatureTemplate.Type.Salamander)
            {
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 4 / 3f; // -25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 4 / 3f; // -25% stun
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 4 / 3f; // -25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 4 / 3f; // -25% stun
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 4f; // -75% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 4f; // -75% stun
            }
            ...
            else if (type == MoreSlugcatsEnums.CreatureTemplateType.ZoopLizard)
            {
                ...
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 2 / 3f; // +50% stun
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 0.8f; // +25% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 0.8f; // +25% stun
            }
            //_____________________
            // Hailstorm lizards
            else if (type == HSEnums.CreatureType.IcyBlueLizard)
            {
                ...
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 0] = 10f; // -90% damage
                temp.damageRestistances[HSEnums.DamageTypes.Cold.index, 1] = 10f; // -90% stun
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 2 / 3f; // +50% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 2 / 3f; // +50% stun
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 1.5f; // -33% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 1.5f; // -33% stun
            }
            else if (type == HSEnums.CreatureType.FreezerLizard)
            {
                ...
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 0] = 0.5f; // +100% damage
                temp.damageRestistances[HSEnums.DamageTypes.Heat.index, 1] = 0.5f; // +100% stun
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 0] = 3f; // -66% damage
                temp.damageRestistances[HSEnums.DamageTypes.Venom.index, 1] = 3f; // -66% stun
            }
            else if (type == HSEnums.CreatureType.GorditoGreenieLizard)
            {
    ...
    public static class WeatherResistances
    {
        ...
        public static void AddCreatureHailResistances()
        {
            ...
            Hail.Add(CreatureTemplate.Type.Centipede, 0.5f);
            Hail.Add(CreatureTemplate.Type.RedCentipede, 0.25f);
            ...
            //Hail.Add(HSEnums.CreatureType.Raven, 0.75f);
        ...
        public static void AddCreatureWindResistances()
        {
            ...
            Wind.Add(MoreSlugcatsEnums.CreatureTemplateType.SpitLizard, 0.4f);
            ...
            // Wind.Add(HailstormEnums.FlurryKing, 0.5f);
