...
internal class OtherCreatureChanges
{
    ...
    private static List<CreatureTemplate.Type> newCreatures = new();
    public static void Hooks()
    {
        newCreatures.Add(HailstormEnums.InfantAquapede);
        newCreatures.Add(HailstormEnums.IcyBlue);
        newCreatures.Add(HailstormEnums.Freezer);
        newCreatures.Add(HailstormEnums.PeachSpider);
        newCreatures.Add(HailstormEnums.Cyanwing);
        newCreatures.Add(HailstormEnums.GorditoGreenie);
        newCreatures.Add(HailstormEnums.Luminescipede);
        newCreatures.Add(HailstormEnums.Chillipede);
    ...
    public static void AbsCtrCWT(On.AbstractCreature.orig_ctor orig, AbstractCreature absCtr, World world, CreatureTemplate temp, Creature realizedCtr, WorldCoordinate pos, EntityID ID)
    {
        ...
        if (absCtr is not null &&
            (IsIncanStory(absCtr.world?.game) || newCreatures.Contains(absCtr.creatureTemplate.type)))
        {
            CustomFlags(absCtr);
        }
    ...
    public static void chickenexterminator(On.Vulture.orig_ctor orig, Vulture vulture, AbstractCreature absCtr, World world)
    {
        ...
        if (IsIncanStory(world.game) && vulture is not null)
        {
            vulture.Destroy();
        }
    ...
    public static void VultureViolence(On.Vulture.orig_Violence orig, Vulture vul, BodyChunk source, Vector2? dirAndMomentum, BodyChunk hitChunk, PhysicalObject.Appendage.Pos appPos, Creature.DamageType dmgType, float dmg, float stun)
    {
        ...
        if (vul?.room is not null && vul.IsMiros && (HSRemix.ScissorhawkNoNormalLasers.Value is true || Random.value < Mathf.Max(0.5f, dmg * 0.3f)) && (IsIncanStory(vul?.room?.game) || HSRemix.HailstormScissorhawksEverywhere.Value is true) && vul.laserCounter == 200 && CWT.CreatureData.TryGetValue(vul, out CreatureInfo cI) && cI.customFunction <= 0)
        {
            cI.customFunction =
    ...
    {
        if (vul?.room?.abstractRoom?.creatures is not null && vul.bodyChunks is not null && (IsIncanStory(vul.room.game) || HSRemix.HailstormScissorhawksEverywhere.Value is true))
        {
    ...
    public static CreatureTemplate.Relationship PleaseThereAreOTHERCREATURESTOEATBESIDESSLUGCATS(On.VultureAI.orig_IUseARelationshipTracker_UpdateDynamicRelationship orig, VultureAI vultureAI, RelationshipTracker.DynamicRelationship dynamRelat)
    {
        ...
        if (vultureAI.vulture.IsMiros && (IsIncanStory(vultureAI.vulture.room.game) || HSRemix.HailstormScissorhawksEverywhere.Value is true))
        {
    ...
    public static void MirosVultureLaserVariation(On.Vulture.orig_Update orig, Vulture vul, bool eu)
    {
        ...
        orig(vul, eu);
        if (vul?.room is not null && vul.IsMiros && (IsIncanStory(vul.room.game) || HSRemix.HailstormScissorhawksEverywhere.Value is true) && CWT.CreatureData.TryGetValue(vul, out CreatureInfo cI))
        {
    ...
    public static void MirosVultureLaserVariationGraphics(On.VultureGraphics.orig_DrawSprites orig, VultureGraphics vg, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        if (vg?.vulture?.room is not null &&
            vg.vulture.IsMiros &&
            !vg.culled &&
            (IsIncanStory(vg.vulture.room.game) || HSRemix.HailstormScissorhawksEverywhere.Value is true) &&
    ...
    {
        if (absCtr is not null && absCtr.creatureTemplate.type == HailstormEnums.Luminescipede && !Luminescipede.WantToHideInDen(absCtr))
        {
            return false;
        }
        return orig(absCtr);
    ...
    public static void HailstormCreatureFlagChecks()
    {
        ...
        IL.AbstractCreature.WantToStayInDenUntilEndOfCycle += IL =>
        {
            ...
            {
                //Plugin.logger.LogError(label.Target.ToString() + " | " + label.Branches.ToString());
            ...
            if (c6.TryGotoNext(
                MoveType.After,
                x => x.MatchLdarg(0),
                x => x.MatchCall<AbstractWorldEntity>("get_Room"),
                x => x.MatchLdfld<AbstractRoom>(nameof(AbstractRoom.world)),
                x => x.MatchLdfld<World>(nameof(World.rainCycle)),
                x => x.MatchLdfld<RainCycle>(nameof(RainCycle.preTimer)),
                x => x.MatchLdcI4(0),
                x => x.MatchBgt(out label)))
            {
                c6.Emit(OpCodes.Ldarg_0);
                c6.EmitDelegate((AbstractCreature absCtr) => absCtr.creatureTemplate.type != HailstormEnums.Luminescipede || absCtr.realizedCreature is null || (absCtr.realizedCreature is not null && absCtr.realizedCreature is Luminescipede lmn && lmn.denMovement == 1));
                c6.Emit(OpCodes.Brfalse, label);
            }
            else
                Plugin.logger.LogError("[Hailstorm] Hook #6 to IL.AbstractCreature.WantToStayInDenUntilEndOfCycle is absolutely not working.");
        };
