...
public class IcyCosmetics
{
    ...
    public static void BumpHawkNegation(On.LizardCosmetics.BumpHawk.orig_ctor orig, BumpHawk BH, LizardGraphics liz, int startSprite)
    {
        ...
        if (liz.lizard.Template.type == HailstormEnums.Freezer || liz.lizard.Template.type == HailstormEnums.IcyBlue)
        {
    ...
    public static void IcyLongHeadScaleColors(On.LizardCosmetics.LongHeadScales.orig_DrawSprites orig, LongHeadScales LHS, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        if (LHS.lGraphics.lizard.Template.type == HailstormEnums.Freezer || LHS.lGraphics.lizard.Template.type == HailstormEnums.IcyBlue)
        {
    ...
    public static void ShortBodyScaleNegation(On.LizardCosmetics.ShortBodyScales.orig_ctor orig, ShortBodyScales SBS, LizardGraphics liz, int startSprite)
    {
        ...
        if (SBS.lGraphics.lizard.Template.type == HailstormEnums.Freezer ||
            SBS.lGraphics.lizard.Template.type == HailstormEnums.IcyBlue)
        {
    ...
    public static void IcyLongBodyScaleSprites(On.LizardCosmetics.LongBodyScales.orig_InitiateSprites orig, LongBodyScales LBS, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
    {
        ...
        bool icyCheck =
            LBS.lGraphics.lizard.Template.type == HailstormEnums.Freezer ||
            (LBS.lGraphics.lizard.Template.type == HailstormEnums.IcyBlue && Random.value < (0.2f + (LBS.lGraphics.lizard.TotalMass - 1.4f) * 4));
    ...
    public static void IcyLongBodyScaleColors(On.LizardCosmetics.LongBodyScales.orig_DrawSprites orig, LongBodyScales LBS, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        if (LBS.lGraphics.lizard.Template.type == HailstormEnums.Freezer || LBS.lGraphics.lizard.Template.type == HailstormEnums.IcyBlue)
        {
            for (int num = LBS.startSprite + LBS.scalesPositions.Length - 1; num >= LBS.startSprite; num--)
            {
                sLeaser.sprites[num].color =
...
public class ArmorIceSpikes : Template
{
    ...
    public ArmorIceSpikes(LizardGraphics lGraphics, int startSprite) : base(lGraphics, startSprite)
    {
        ...
        spikeLength = lGraphics.BodyAndTailLength * (liz.Template.type == HailstormEnums.Freezer? 0.3f : 0.5f);
    ...
    public static void DropCrystals(Lizard liz, Vector2 crystalPos, int crystalType)
    {

        Color lizEffectColor2 = HailstormLizards.SecondaryFreezerColor(liz.graphicsModule as LizardGraphics);

        for (int particle = 0; particle < 13; particle++)
        {
            if (particle % 2 == 0) liz.room.AddObject(new HailstormSnowflake(crystalPos, Custom.RNV() * Random.value * 12f, liz.effectColor, lizEffectColor2));
            else liz.room.AddObject(new PuffBallSkin(crystalPos, Custom.RNV() * Random.value * 12f, liz.effectColor, lizEffectColor2));
        }
        ...
        AbstractIceCrystal absIceCrs = new AbstractIceCrystal(liz.room.world, liz.abstractCreature.pos, liz.room.game.GetNewID());
        absIceCrs.lizCrystalType = crystalType;
        absIceCrs.baseColor = liz.effectColor;
        absIceCrs.accentColor = lizEffectColor2;
        liz.room.abstractRoom.AddEntity(absIceCrs);
        absIceCrs.RealizeInRoom();
        absIceCrs.realizedObject.firstChunk.HardSetPosition(crystalPos);
        absIceCrs.realizedObject.firstChunk.vel.y = Random.Range(3f, 4.5f);
        absIceCrs.realizedObject.firstChunk.vel.x *= rotationSide;

        IceCrystal iceCrs = absIceCrs.realizedObject as IceCrystal;
        iceCrs.setRotation = Custom.RNV();
        iceCrs.SetRandomSpin();     
...
public class IceSpikeTuft : LongBodyScales
{
    ...
    {
        //------------------------------------------------------
        bool icyBlue = lGraphics.lizard.Template.type == HailstormEnums.IcyBlue;
        bool freezer = lGraphics.lizard.Template.type == HailstormEnums.Freezer;
        rigor = 0f;
    ...
    private void TwoLines(float startPoint, float maxLength, float spacingScale, float randomValue)
    {
        ...
        if (lGraphics.lizard.abstractCreature.creatureTemplate.type == HailstormEnums.Freezer)
        {
            num3 = (randomValue < 0.25f)? 6.25f : 7.5f;
        }
        if (lGraphics.lizard.abstractCreature.creatureTemplate.type == HailstormEnums.IcyBlue)
        {
...
public class IcyRhinestones : BodyScales
{
    ...
    public IcyRhinestones(LizardGraphics lGraphics, int startSprite) : base(lGraphics, startSprite)
    {
        ...
        if (Random.value < (lizType == HailstormEnums.Freezer? 0.6f : 0.4f))
        {
            stoneType = "pixel";
        }
