...
public class Luminescipede : InsectoidCreature, IPlayerEdible
{
    ...
    public Luminescipede(AbstractCreature absLmn, World world) : base(absLmn, world)
    {
    ...
    {   // If Lumins from different dens try to carry prey together, they may get stuck trying to go in different directions.
        ...
        for (int g = 0; g < grasps[0].grabbed.grabbedBy.Count; g++)
        {
            ...
            if (grasp.grabber is null ||
                grasp.grabber is not Luminescipede otherLmn ||
                otherLmn == this ||
    ...
    public virtual void UseitemUpdate()
    {
        ...
        if (!grabbing)
        {
            if (AI.VisualContact(MainChunkOfObject(useItem).pos))
            {
                if (Behavior != Flee)
                {
                    GlowState.ChangeBehavior(Hunt, 0);
    ...
    public virtual bool WillingToDitchCurrentPrey(PhysicalObject newTarget)
    {
        ...
        float oldItemAppeal = CurrentPreyRelationIntensity;
        float newItemAppeal = AI.ObjRelationship(newTarget.abstractPhysicalObject).intensity;

        if (!GrabbingItem(currentPrey))
        {
            oldItemAppeal *= 0.2f + (0.8f * AI.VisualScore(MainChunkOfObject(currentPrey).pos, 0));
            if (Behavior == Flee)
            {
                oldItemAppeal *= AI.VisualScore(MainChunkOfObject(currentPrey).pos, 0);
            }
        }
        if (!GrabbingItem(newTarget))
        {
            newItemAppeal *= 0.2f + (0.8f * AI.VisualScore(MainChunkOfObject(newTarget).pos, 0));
            if (Behavior == Flee)
            {
                newItemAppeal *= AI.VisualScore(MainChunkOfObject(newTarget).pos, 0);
            }
        }

        if (oldItemAppeal >= newItemAppeal)
        {
            return false;
        }
    ...
    public virtual void RushAlert()
    {
        ...
        {
            if (absCtr?.realizedCreature is not Luminescipede otherLmn || otherLmn == this || !AI.VisualContact(otherLmn.DangerPos) || otherLmn.Behavior != Hide)
            {
                continue;
            }
