...
public class SnowcuttleCreature : InsectoidCreature
{
    ...
    public override void Update(bool eu)
    {
        ...
        if ((base.State as HealthState).health < 0.5f && Random.value > (base.State as HealthState).health && Random.value < 1f / 3f)
        {
            Stun(4);
            if ((base.State as HealthState).health <= 0f && Random.value < 0.25f)
            {
                Die();
            }
        }
    ...
    private void Act()
    {
        ...
        if (safariControlled && (movementConnection is not null || !AllowableControlledAIOverride(movementConnection.type)))
        {
            ...
            if (inputWithDiagonals.HasValue)
            {
                ...
                {
                    movementConnection = new MovementConnection(type, room.GetWorldCoordinate(base.mainBodyChunk.pos), room.GetWorldCoordinate(base.mainBodyChunk.pos + new Vector2((float)inputWithDiagonals.Value.x, (float)inputWithDiagonals.Value.y) * 40f), 2);
                }
                if (inputWithDiagonals.Value.jmp && !lastInputWithDiagonals.Value.jmp && chargeCounter == 0)
                {
                    ...
                    {
                        Charge(base.mainBodyChunk.pos + new Vector2((float)inputWithDiagonals.Value.x, (float)inputWithDiagonals.Value.y) * 40f);
                    }
                    else
                    {
        ...
        if (flying)
        {
            ...
            bool flag = false;
            if (movementConnection == null || Climbable(movementConnection.DestTile) || Climbable(room.GetTilePosition(base.mainBodyChunk.pos)))
            {
                if (room.aimap.getAItile(base.bodyChunks[0].pos).narrowSpace)
                {
                    flag = true;
                }
                else if (room.aimap.getAItile(base.bodyChunks[0].pos).terrainProximity == 1 && room.aimap.getAItile(base.bodyChunks[1].pos).terrainProximity == 1 && (movementConnection == null || room.aimap.getAItile(movementConnection.destinationCoord).terrainProximity == 1))
                {
                    flag = true;
                }
    ...
    private void GrabbedByPlayer()
    {
        ...
        if (player.input[0].x == 0 && player.input[0].y == 0 && stamina == 1f)
        {
            ...
            BodyChunk obj3 = base.bodyChunks[0];
    ...
    public void CarryObject()
    {
        ...
        Vector2 val = Custom.DirVec(base.mainBodyChunk.pos, base.grasps[0].grabbedChunk.pos);
        ...
        BodyChunk bodyChunk2 = base.mainBodyChunk;
        ...
        BodyChunk grabbedChunk2 = base.grasps[0].grabbedChunk;
    ...
    public bool Climbable(IntVector2 tile)
    {
        if (safariControlled)
        {
            if (!inputWithDiagonals.HasValue || !inputWithDiagonals.Value.pckp || room.aimap.getAItile(tile).terrainProximity != 1)
            {
                return room.aimap.getAItile(tile).acc == AItile.Accessibility.Climb;
            }
            return true;
        }
        if (room.aimap.getAItile(tile).terrainProximity != 1)
        {
            return room.aimap.getAItile(tile).acc == AItile.Accessibility.Climb;
        }
        return true;
    ...
    public bool TryToGrabPrey(PhysicalObject prey)
    {
        ...
        {
            if (Custom.DistLess(base.mainBodyChunk.pos, prey.bodyChunks[i].pos, Mathf.Max(num, prey.bodyChunks[i].rad + base.mainBodyChunk.rad + 3f)))
            {
                num = Vector2.Distance(base.mainBodyChunk.pos, prey.bodyChunks[i].pos);
        ...
        for (int j = 0; j < 2; j++)
        {
            BodyChunk obj = base.bodyChunks[j];
            obj.vel *= 0.75f;
            BodyChunk obj2 = base.bodyChunks[j];
    ...
    private void TakeOff(Vector2 dir)
    {
        ...
        flyingPower = 0.5f;
        room.PlaySound(SoundID.Cicada_Wings_TakeOff, mainBodyChunk, false, 1f, iVars.wingSoundPitch);
    }
    ...
    private void Land()
    {
        waitToFlyCounter = 0;
        flying = false;
        room.PlaySound(SoundID.Cicada_Landing, mainBodyChunk);
    }
    ...
    public override void SpitOutOfShortCut(IntVector2 pos, Room newRoom, bool spitOutAllSticks)
    {
        ...
        {
            bodyChunks[i].pos = newRoom.MiddleOfTile(pos) - spitoutAngle * (-1.5f + (float)i) * 15f;
    ...
    public override void RecreateSticksFromAbstract()
    {
        for (int i = 0; i < abstractCreature.stuckObjects.Count; i++)
        {
                ...
                grip.B.realizedObject is not null)
            {
                ...
                grip.B.realizedObject.Grabbed(base.grasps[grip.grasp]);
            }
        }
    }
}
